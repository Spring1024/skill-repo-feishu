...
| 场景 | 说明 |
|------|------|
| **交换大文件** | Bot 之间传递 SQL 脚本、配置、PRD 文档等，避免在群聊中粘贴大段内容 |
| **共享项目文档** | 上传项目文档到指定目录，群内所有成员可查看 |
| **版本管理** | 更新文件后自动覆盖旧版本，保留最新内容 |
| **文件分发** | 上传后自动设置公开权限，无需手动配置 |

### 协作流程

1. **生成文件** → 2. **描述文件**（群聊中简要说明） → 3. **上传云空间** → 4. **共享链接** → 5. **对方下载**
