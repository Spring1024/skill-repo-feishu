...
# Detect <at> tags for Bot-to-Bot collaboration — captures open_id and display name
...
def _build_at_post_payload(content: str) -> str:
    """Build a Feishu post payload with proper <at> tag rendering.

    Feishu's post-type messages support <at> tags only when they appear as
    individual elements in the content array, NOT inside md-formatted text.
    """
    matches = list(_AT_TAG_RE.finditer(content))
    if not matches:
        return _build_markdown_post_payload(content)  # fallback
    ...
    for match in matches:
        ...
        before_text = content[prev_end:match.start()].strip()
        if before_text:
            rows.append([{"tag": "md", "text": before_text}])
    ...
    remaining = content[prev_end:].strip()
    if remaining:
        rows.append([{"tag": "md", "text": remaining}])
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def _on_bot_added_to_chat(self, data: Any) -> None:
        ...
        When the bot is added to a new group, schedule a background thread
        to collect introduction data and send the message asynchronously.
        """
        event = getattr(data, "event", None)
        ...
        # 检查是否已经发送过介绍
        intro_sent = getattr(self, "_introduced_chats", set())
        if chat_id in intro_sent:
            logger.info("[Feishu] Already introduced to chat %s, skipping", chat_id)
            return
        
        # 另起线程异步执行，不阻塞事件循环
        thread = threading.Thread(
            target=self._send_group_introduction_async,
            args=(chat_id,),
            daemon=True,
            name=f"feishu-intro-{chat_id}",
        )
        thread.start()
    
    def _send_group_introduction_async(self, chat_id: str) -> None:
        """异步发送群介绍：先收集数据，再调用飞书 API。

        流程：
        1. 通过飞书 API 获取 open_id
        2. 读取并解析 SOUL.md
        3. 生成介绍文本
        4. 调用飞书 API 发送消息
        """
        try:
            logger.info("[Feishu] Starting group introduction for chat %s", chat_id)
            
            # Step 1: 获取 open_id
            open_id = self._get_bot_open_id()
            if not open_id:
                logger.error("[Feishu] Failed to get open_id for chat %s", chat_id)
                return
            
            # Step 2: 读取并解析 SOUL.md
            soul_content = self._read_soul_content()
            if not soul_content:
                logger.error("[Feishu] Failed to read SOUL.md for chat %s", chat_id)
                return
            
            identity = self._extract_identity(soul_content)
            identity["open_id"] = open_id
            
            # Step 3: 生成介绍文本
            introduction = self._generate_introduction(identity)
            logger.info("[Feishu] Generated introduction (%d chars) for chat %s", len(introduction), chat_id)
            
            # Step 4: 调用飞书 API 发送消息
            self._send_introduction_to_chat(chat_id, introduction)
            
            # 记录已发送
            if not hasattr(self, "_introduced_chats"):
                self._introduced_chats = set()
            self._introduced_chats.add(chat_id)
            logger.info("[Feishu] Group introduction sent successfully for chat %s", chat_id)
            
        except Exception as e:
            logger.error("[Feishu] Error sending group introduction for chat %s: %s", chat_id, e, exc_info=True)
    
    def _get_bot_open_id(self) -> Optional[str]:
        """通过飞书 API 获取当前 Bot 的 open_id。"""
        try:
            ...
            base_url = "https://open.larksuite.com" if self._settings.domain_name == "lark" else "https://open.feishu.cn"
            ...
            with _httpx.Client(timeout=30) as client:
                ...
                if data.get("code") != 0:
                    logger.warning("[Feishu] Failed to get bot info: %s", data.get("msg"))
                    return None
                
                bot_info = data.get("bot", {})
                open_id = bot_info.get("open_id", "")
                app_name = bot_info.get("app_name", "")
                logger.info("[Feishu] Bot info: %s (open_id: %s)", app_name, open_id)
                return open_id
                
        except Exception as e:
            logger.error("[Feishu] Error getting bot open_id: %s", e)
            return None
    ...
    def _get_tenant_token_sync(self) -> Optional[str]:
        ...
        try:
            ...
            with _httpx.Client(timeout=30) as client:
                ...
                if data.get("code") != 0:
                    logger.error("[Feishu] Failed to get token: %s", data.get("msg"))
                    return None
                
                return data["tenant_access_token"]
                
        except Exception as e:
            logger.error("[Feishu] Error getting token: %s", e)
            return None
    
    def _read_soul_content(self) -> Optional[str]:
        """读取 SOUL.md 文件内容。"""
        try:
            search_paths = [
            ...
            logger.warning("[Feishu] SOUL.md not found in any of: %s", search_paths)
            return None
                
        except Exception as e:
            logger.error("[Feishu] Error reading SOUL.md: %s", e)
            return None
    
    def _extract_identity(self, soul_content: str) -> dict:
        """从 SOUL.md 内容中提取身份信息（通用型解析）。"""
        identity = {
            "name": "未知助手",
            "responsibility": "",
            "team_role": "",
    ...
    def _generate_introduction(self, identity: dict) -> str:
        """根据身份信息生成介绍文本。"""
    ...
    def _send_introduction_to_chat(self, chat_id: str, content: str) -> None:
        """调用飞书 API 发送介绍消息。"""
        try:
            import httpx as _httpx
            ...
            token = self._get_tenant_token_sync()
            if not token:
                logger.error("[Feishu] Cannot send: failed to get token")
                return
            ...
            with _httpx.Client(timeout=30) as client:
                ...
                if result.get("code") != 0:
                    logger.error("[Feishu] Failed to send message: %s", result.get("msg"))
                    return
                
                logger.info("[Feishu] Message sent: %s", result["data"]["message_id"])
                
        except Exception as e:
            logger.error("[Feishu] Error sending message: %s", e)
    ...
    def _build_outbound_payload(self, content: str) -> tuple[str, str]:
        ...
        # Detect <at> tags — requires rich-text post format, not markdown
